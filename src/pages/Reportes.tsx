...
import { useAuth } from '../context/AuthContext'; // Asegúrate de que este import sea correcto según tu proyecto
import { NotificationBell } from '../components/NotificationBell';

export const Reportes: React.FC = () => {
  const { roles } = useAuth(); // Obtener los roles del usuario desde el contexto
  const isMecanico = roles.includes('mecanico'); // Verificar si el usuario es mecánico
  ...
  const [showNotificationDialog, setShowNotificationDialog] = useState(false); // Controlar el diálogo de notificaciones
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        toast.current?.show({ severity: 'error', summary: 'Error', detail: error instanceof Error ? error.message : 'Error al cargar los datos', life: 3000 });
      }
    };
    loadData();
  }, []);
  ...
  const handleNotificationClick = () => {
    setShowNotificationDialog(true); // Abrir el diálogo al hacer clic en la campanita
  };
  ...
  const reporteCostosPorVehiculo = () => {
    return vehiculos.map((vehiculo) => {
      ...
      return { vehiculoId: vehiculo.id, marca: vehiculo.marca, modelo: vehiculo.modelo, totalCosto };
    });
  };
  ...
  const reporteCostosPorTaller = () => {
    return talleres.map((taller) => {
      const totalCosto = registrosServicio
        ...
        .reduce((acc: number, registro: RegistroServicio) => acc + Number(registro.costo || 0), 0);
      return { tallerId: taller.id, nombreTaller: taller.nombre, totalCosto };
    });
  };
  ...
  const estadisticasFrecuenciaMantenimiento = () => {
    return vehiculos.map((vehiculo) => {
      ...
      return { vehiculoId: vehiculo.id, marca: vehiculo.marca, modelo: vehiculo.modelo, cantidadMantenimientos };
    });
  };
  ...
  const tiempoPromedioEntreServicios = () => {
    return vehiculos.map((vehiculo) => {
      ...
      return { vehiculoId: vehiculo.id, marca: vehiculo.marca, modelo: vehiculo.modelo, promedio: promedio / (1000 * 3600 * 24) };
    });
  };
  ...
  return (
    <div>
      ...
      {/* Mostrar la campanita solo para mecánicos */}
      {isMecanico && (
        <div style={{ marginBottom: '1rem' }}>
          <NotificationBell count={vehiculosRequierenRevision.length} onClick={handleNotificationClick} />
        </div>
      )}

      {/* Diálogo que se abre al hacer clic en la campanita */}
      <Dialog header="Vehículos que requieren revisión" visible={showNotificationDialog} style={{ width: '50vw' }} onHide={() => setShowNotificationDialog(false)}>
